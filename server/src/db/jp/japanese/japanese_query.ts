...
export interface IWordRow extends RowDataPacket {
  ...
  //GETTERS
  export function getAllJapaneseWords() {
    return SelectQuery<IWordRow>('SELECT * FROM japanese_words;');
  }

  export function getOneJapaneseWord(id: string) {
    return SelectQuery<IWordRow>('SELECT * FROM japanese_words where id = ' + id + ';');
  }
